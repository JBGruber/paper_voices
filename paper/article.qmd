...
These data are matched and the hypotheses are tested through multilevel logistic regression models, with the presence or absense of a frame as dependant, and the above described factors that are assumed to shape protest reporting as independent variables.
...
Table @tab:outlets presents the selection of newspaper outlets, strategically chosen to contain variety on the two important factors: 
...
I retrieved articles from LexisNexis, utilizing keywords such as "protest" and "demonstration" and several several misspellings.
...
To maintain theoretical integrity, we opted to limit our data to domestic protests, as studies have indicated notable differences in the coverage of non-domestic protests [@boyleAdherenceProtestParadigm2012].
...
outlets |>  
  knitr::kable(format = "latex",
        booktabs = TRUE,
        linesep = "",
        caption = "Selection of Newspaper Outlets for Data Collection",
        longtable = FALSE,
        escape = FALSE) |>  
  kableExtra::add_footnote(footnotes, notation = "alphabet", threeparttable = TRUE)
```
...
One article can contain several frames, or none.
In short, I frames were identified by coding the frame elements found in @entman1993's definition of framing (problem definition, causal interpretation, moral evaluation, and treatment recommendation) in 500 randomly selected artiocle and combining the coded elements to frames using factor analysis.
They were then coded using supervised machine learning models trained on the manually coded material.
...
then I used the variables "Protest Locations", "Protest Group Identity" and "Protester Demands" to extract suitable keywords [see Appendix @app-eventdata].
...
The newspaper ideology is known; however, I had to code the ideology of the protest myself based on the information in the MMP dataset and in other articles about an event.^[See Appendix @app-eventdata for the specific coding.]
...
Using the variables listed in @tab-variables, I fitted multilevel logistic regression models (one for each frame) to establish the influence of the different factors on reporting.
